...
/**
 * Sample data using obviously fictitious names so it can never be confused
 * with real participant data. Safe to share with prospective teams.
 */
const SAMPLE_DATA: Participant[] = [
  // Team Example A — high agreement team
  ...
  // Team Example B — diverse competency team
...
const ParticipantFeedbackGenerator = () => {
  ...
  const calculateTeamStats = (participants: Participant[]) => {
    ...
    Object.keys(teams).forEach(team => {
      ...
      const teamMeanOverall = teams[team].members.reduce((sum, m) => sum + (m.overallScore ?? 0), 0) / teams[team].count;
